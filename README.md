...
The training and prediction datasets used were prepared with the matminer library, pymatgen, and Materials Project API<sup>4-6</sup>. All the training and prediction datasets contain 145 compositional attributes extracted using Matminer. Users may refer to the matminer documentation as a guide (Link: https://hackingmaterials.lbl.gov/matminer/index.html) The chemical composition is used as the required input to extract the aforementioned features. It is recommended that users prepare the training and prediction datasets directly from the respective sources mentioned below:

The prediction datasets are prepared and extracted from the Materials Project API<sup>4</sup>. The general screening criteria used to prepare the prediction datasets are as follows:
